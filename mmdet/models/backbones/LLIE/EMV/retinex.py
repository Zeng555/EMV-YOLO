...
class DecomNet(nn.Module):
    def __init__(self, layer_num, channel=64, kernel_size=3):
        ...
        self.shallow_feature_extraction = nn.Conv2d(4, channel, kernel_size * 3, padding='same')
        
        self.activated_layers = nn.ModuleList([
            nn.Conv2d(channel, channel, kernel_size, padding='same')
            for _ in range(layer_num)
        ])
        
        self.recon_layer = nn.Conv2d(channel, 4, kernel_size, padding='same')
...
class ReDiff_retinex(nn.Module):
